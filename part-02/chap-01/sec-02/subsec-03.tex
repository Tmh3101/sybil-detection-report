\subsubsection{2.3. Mạng nơ-ron đồ thị}
\indent\indent Mạng nơ-ron đồ thị là một lớp mô hình học sâu chuyên biệt được thiết kế để khai thác dữ liệu trong không gian phi Euclid. Theo Zhang và cộng sự \cite{zhang2019graph}, mục tiêu của GNN là học các vector biểu diễn nút sao cho bảo tồn được cả đặc trưng của nút và cấu trúc liên kết của mạng lưới. Trong bài toán này, GNN đóng vai trò là bộ não trích xuất hành vi của các cụm Sybil từ CMG.
...
Cơ chế hoạt động cốt lõi của GNN là truyền thông điệp (\textit{Message Passing}). Tại mỗi tầng $l$, đặc trưng của nút $v$ được cập nhật thông qua việc tổng hợp thông tin từ tập hàng xóm $N(v)$:
...
Trong đó $h_v^{(l)}$ là trạng thái ẩn của nút tại tầng $l$ trong kiến trúc mô hình.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{images/part-02/chap-01/GNNMessagePassing.png}
    \vspace{0.1cm}
    \caption{Cơ chế truyền tin cơ bản trong mạng GNN}
    \label{fig:GNN_MP}
\end{figure}

\textit{2.3.1. Các kiến trúc mạng nơ-ron đồ thị thực nghiệm}\vspace{0.2cm}

Với đặc thù phức tạp của CMG, đề tài không giới hạn ở một kiến trúc duy nhất mà thiết lập một hệ thống thử nghiệm toàn diện. Năm kiến trúc GNN tiên tiến được triển khai và đánh giá để tìm ra mô hình tối ưu nhất:

\begin{itemize}[label={--}, itemsep=5pt]
    \item \textbf{Graph Attention Networks (GATv2):} Đóng vai trò là kiến trúc chủ đạo của hệ thống. GAT nguyên bản \cite{velickovic2017graph} tạo đột phá bằng cơ chế tự chú ý, nhưng bộc lộ giới hạn trong chú ý tĩnh, khiến mức độ quan trọng của láng giềng bị đánh giá toàn cục. Khắc phục rào cản này, Brody và cộng sự \cite{brody2021attentive} đã đề xuất GATv2 với cơ chế chú ý động. Nhờ cơ chế này của GATv2, mô hình trở nên linh hoạt hơn trong việc phân bổ trọng số, từ đó bước đầu học được cách dồn sự chú ý vào các liên kết mang tín hiệu quyết định. Bản chất toán học của kiến trúc GATv2 nguyên bản được thể hiện qua các bước sau:
    \begin{itemize}[label={+}, itemsep=2pt]
        \item \textbf{Hệ số chú ý chưa chuẩn hóa} $e_{ij}$ được tính bằng cách ghép nối vector đặc trưng của hai nút, biến đổi tuyến tính qua ma trận chú ý và áp dụng hàm phi tuyến trước khi nhân với vector chú ý:

        \begin{center}
        \(
            e_{ij} = \vec{a}^T \text{LeakyReLU}\left( \mathbf{W}_{att} [h_i \| h_j] \right)
        \)
        \end{center}

        Trong đó: $h_i, h_j$ là vector đặc trưng ẩn của nút gốc $i$ và láng giềng $j$; $\mathbf{W}_{att}$ là ma trận trọng số chú ý học được; $\vec{a}$ là vector chú ý.

        \item \textbf{Trọng số chú ý} $\alpha_{ij}$ được chuẩn hóa với hàm softmax:
        ...
        \item \textbf{Vector đầu ra} được tính bằng cách tổng có trọng số các đặc trưng láng giềng đã biến đổi tuyến tính, sau đó đưa qua hàm phi tuyến ELU:

        \begin{center}
        \(
            \vec{h}'_i = \text{ELU} \left( \sum_{j \in N_i} \alpha_{ij} \mathbf{W}_v\vec{h}_j \right)
        \)
        \end{center}

        Trong đó $\mathbf{W}_v$ là ma trận trọng số đặc trưng học được.
        
        \item Mô hình tích hợp \textit{Multi-head Attention} tương tự Transformer \cite{vaswani2017attention} để ổn định quá trình học, thực hiện ghép nối ở tầng ẩn và lấy trung bình ở tầng đầu ra.
    \end{itemize}

    \item \textbf{Graph Convolutional Networks (GCN):} Dựa trên công trình nền tảng của Kipf và Welling \cite{kipf2017semi}, GCN sử dụng phép xấp xỉ bậc nhất của tích chập phổ đồ thị để lan truyền và tổng hợp thông tin. Khác biệt cốt lõi với cơ chế chú ý động của GATv2 là GCN vận hành dựa trên một bộ lọc tĩnh, nơi trọng số lan truyền được cố định dựa trên cấu trúc hình học của đồ thị thay vì nội dung đặc trưng nút. Tính chất này được thể hiện qua công thức lan truyền đa tầng ở dạng ma trận:
    ...
    Cơ chế chú ý tĩnh và sự hạn chế của GCN nguyên bản được bộc lộ rõ nhất khi xét quá trình lan truyền thông điệp từ láng giềng $j$ đến nút $i$ ở cấp độ từng nút:
    ...
    \item \textbf{Graph Isomorphism Network with Edge features (GINE):} Đóng vai trò là kiến trúc đại diện cho trường phái phân tích cấu trúc tô-pô chuyên sâu. Nguyên bản của kiến trúc này là mạng GIN (\textit{Graph Isomorphism Network}) được đề xuất bởi Xu và cộng sự \cite{xu2018powerful}. Sử dụng phép tổng kết hợp với mạng nơ-ron đa tầng. Tuy nhiên, GIN nguyên bản bộc lộ giới hạn lớn khi không có khả năng xử lý các đặc trưng cạnh. Từ đó biến thể GINE được giới thiệu thông qua nghiên cứu của Hu và cộng sự \cite{hu2019strategies}. Công thức truyền tin được tùy biến để nhúng trực tiếp trọng số cạnh $e_{uv}$ vào thông điệp của láng giềng trước khi thực hiện phép gộp:
    ...
    \item \textbf{Graph Transformer Networks:} Được chuyển thể từ cơ chế tự chú ý mang tính bước ngoặt của mô hình Transformer thuần túy trong lĩnh vực xử lý ngôn ngữ tự nhiên \cite{vaswani2017attention}, kiến trúc này được tùy biến cho đồ thị thông qua mạng truyền tin hợp nhất UniMP \cite{shi2020masked}. Khác biệt hoàn toàn với các bộ lọc cấu trúc hay bộ thống kê, Graph Transformer thực hiện tổng hợp láng giềng dựa trên cơ chế chú ý tích vô hướng có tỷ lệ:
    \begin{center}
    \(
...
\textit{2.3.2. Tối ưu hóa tham số cho đồ thị đa quan hệ}\vspace{0.2cm}

Trong kiến trúc đồ thị đa quan hệ có trọng số, các thực thể tương tác thông qua 14 loại quan hệ gốc. Việc biểu diễn trực tiếp các cạnh này bằng phương pháp mã hóa One-hot 14 chiều khi đưa vào mạng GNN có thể dẫn đến tình trạng bùng nổ tham số và overfitting, đặc biệt trên các tập dữ liệu đồ thị có đặc tính thưa thớt. Để giải quyết rào cản này, nghiên cứu thực hiện chiến lược tối ưu hóa tham số toàn diện từ cấp độ cấu trúc dữ liệu đến lõi kiến trúc mô hình:
...
\begin{itemize}[label={--}, itemsep=5pt]
        ...
        \begin{center}
        \(
            e_{ij} = \vec{a}^T \text{LeakyReLU}\left( \mathbf{W}_{att} [h_i \| h_j] + \mathbf{W}_e w_{ij} \right)
         \)
        \end{center}
        ...
        \begin{itemize}[label={+}, itemsep=4pt]
            ...
            \item \textbf{GCN:} Để xóa bỏ cơ chế cào bằng theo cấu trúc, hệ thống can thiệp vào luồng truyền tin bằng cách truyền trực tiếp tensor \texttt{edge\_weight} 1D. Ma trận kề nhị phân nguyên thủy bị vô hiệu hóa, hệ số chuẩn hóa tĩnh được thay thế bằng trọng số hành vi, dẫn đến công thức cập nhật mới:
            \begin{center}
            \(
            ...
            \item \textbf{GINE:} Nguyên bản GINE yêu cầu đặc trưng cạnh và nút phải đồng nhất về không gian. Để khắc phục sự bất đồng bộ giữa vector nút đa chiều $h_j$ và trọng số vô hướng $w_{ij}$, hệ thống ép kiểu tensor và thiết lập chiêu của đặc trưng cạnh là 1. Thao tác này kích hoạt ma trận phóng chiếu nội tại $\mathbf{W}_e$, đẩy $w_{ij}$ vào cùng không gian ẩn và cộng trực tiếp vào láng giềng trước hàm kích hoạt phi tuyến:
            \begin{center}
            \(
            ...
            \item \textbf{PNA:} Để khắc phục điểm mù về thông tin giao dịch của các toán tử thống kê, hệ thống áp dụng cơ chế dung hợp sớm thông qua mạng tiền xử lý (kí hiệu là $\text{MLP}_{pre}$). Công thức truyền tin được tùy chỉnh mở rộng:
            \begin{center}
            \(
            ...
            \item \textbf{Graph Transformer Networks:} Để đối phó với kịch bản tấn công sao chép. Kế thừa cơ chế của mạng UniMP, hệ thống tiêm trực tiếp trọng số $w_{ij}$ vào cả ma trận Khóa (\textit{Key}) và Giá trị (\textit{Value}). Công thức tổng hợp láng giềng được cập nhật thành:
            \begin{center}
            \(
