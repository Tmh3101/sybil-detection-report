...
Quy trình bắt đầu bằng việc sử dụng mô hình GAE để nén không gian đặc trưng hỗn hợp 396 chiều xuống không gian nhỏ hơn. Khác với các phương pháp giảm chiều thông thường, GAE bảo toàn đồng thời cả đặc trưng nút và cấu trúc liên kết đa tầng của đồ thị.\vspace{0.3cm}

Tiếp theo, thuật toán K-Means được áp dụng trên các vector embeddings để xác định các phân vùng hành vi tương đồng. Trong kỹ thuật học không giám sát, việc xác định số lượng cụm ($K$) tối ưu là yếu tố quyết định đến độ chính xác của nhãn giả định. Đề tài không cố định $K$ mà tiến hành tìm kiếm tối ưu trên từng quy mô dữ liệu. Quá trình đánh giá sử dụng kết hợp hai thước đo: \textit{Hệ số Silhouette} và \textit{Chỉ số Davies-Bouldin}.
...
\begin{itemize}[label={--}, itemsep=1pt]
    \item \textbf{Phạm vi Tuần:} Chọn $\mathbf{K=14}$, tại đây hệ số Silhouette đạt đỉnh cục bộ ($0.4901$) và Davies-Bouldin ở mức hợp lý ($0.8398$).
    \item \textbf{Phạm vi Tháng:} Đạt sự đồng thuận cao giữa hai thước đo tại $\mathbf{K=122}$ (Silhouette cực đại ở $0.6391$ và Davies-Bouldin chạm đáy tại $0.7452$).
    \item \textbf{Phạm vi Quý:} Trong không gian tìm kiếm $K \in [160, 180]$, mặc dù Silhouette ưu tiên $K=167$, hệ thống quyết định chọn $\mathbf{K=179}$ vì chỉ số Davies-Bouldin đạt mức cực tiểu sâu nhất ($1.1087$), giúp các cấu trúc Sybil phức tạp được bó chặt tốt hơn.
\end{itemize}

\indent\indent Điểm đặc thù trong kết quả phân cụm là hệ thống đã lựa chọn các giá trị $K$ tương đối lớn, dẫn đến quy mô trung bình của mỗi cụm khá nhỏ (biến thiên từ 11 đến 28 nút/cụm). Để biện minh cho không gian khảo sát tham số này, nghiên cứu áp dụng trực tiếp công thức ước lượng \textit{Core-Whisker} đã được thiết lập tại Mục 2.5.1 (Chương 1).\vspace{0.3cm}

Áp dụng công thức đối với tập dữ liệu Quý ($N=4997$) và giả định kích thước cụm lính ảo trung bình $k_{target} \approx 11.5$, hệ thống thu được ước lượng không gian tìm kiếm:
\begin{equation}
    K_{Quy} \approx \frac{0.4 \times 4997}{11.5} \approx 173
\end{equation}

Giá trị ước lượng lý thuyết này hoàn toàn nằm trong biên độ khảo sát thực nghiệm $K \in [160, 180]$ và hội tụ tối ưu tại điểm $K=179$. Tính toán tương tự cho tập dữ liệu Tháng ($N=1416$) cũng định hướng vùng khảo sát quanh ngưỡng $K \approx 113$, rất gần với điểm hội tụ thực tế $K=122$. Sự đồng nhất giữa ước lượng toán học và kết quả chạy thuật toán là minh chứng rành mạch cho việc lựa chọn giá trị $K$ lớn là một chiến lược cấu hình tham số có chủ đích nhằm phản ánh chính xác cấu trúc vi mô của các tổ chức lính ảo trên không gian Web3.\vspace{0.3cm}

Khi không gian biểu diễn đã được phân tách thành $K$ cụm tối ưu dựa trên cơ sở toán học trên, hệ thống tiến hành áp dụng hệ luật chuyên gia định lượng để chấm điểm rủi ro. Dựa trên mức độ vi phạm, hàng trăm vi cụm này sẽ được ánh xạ và hợp nhất về 04 nhóm nhãn vĩ mô chính:
\begin{itemize}[label={--}, itemsep=1pt]
    \item \textbf{BENIGN:} Thực thể có hành vi phân tán tự nhiên, điểm tin cậy cao và không trùng lặp các chỉ số định danh.
    \item \textbf{LOW\_RISK:} Các tài khoản mới hoặc có tần suất hoạt động thấp, chưa bộc lộ rõ rệt cấu trúc liên kết dị thường.
    \item \textbf{HIGH\_RISK:} Nhóm có dấu hiệu vận hành tập trung (Co-owner) nhưng nội dung hồ sơ vẫn có sự đa dạng nhất định.
    \item \textbf{MALICIOUS:} Các mạng lưới Sybil chuyên nghiệp với cấu trúc hình sao quanh các ví sở hữu và sự rập khuôn tuyệt đối về nội dung/định danh.
\end{itemize}
...
\begin{itemize}[label={--}, itemsep=2pt]
    \item \textbf{Tính sở hữu:} Xác thực tỷ lệ đồng sở hữu. Các cụm \texttt{MALICIOUS} điển hình thường có tỷ lệ trùng ví trên $40\%$, thậm chí đạt $100\%$ ở các chiến dịch lính ảo quy mô lớn.
